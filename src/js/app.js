...
function checkVideo() {
    ...
    if(document.querySelector('.wants-video')) {
        video = true;
    }
...
function runVideo() {
    var videos = document.getElementsByClassName('wants-video');
    for(var i=0; i < videos.length; i++) {
        var src = videos[i].dataset.video;
        var videoEl = document.createElement('video');
        videoEl.setAttribute('src', src);
        videoEl.setAttribute('class', 'video-player');
        videoEl.setAttribute('mute', '');
        videoEl.setAttribute('loop', '');
        var parent = videos[i].parentElement;
        parent.insertBefore(videoEl, videos[i]);
        videos[i].classList.add('video-added');
    }
}
...
function playVideo(){
    ...
    document.addEventListener('scroll', function(){
        for(var i=0; i < videos.length; i++) {
            if(elementInViewport(videoPlayer[i])){
                videoPlayer[i].play();
            }else {
                videoPlayer[i].pause();
            }
...
function playYoutube() {
    ...
    var src = 'https://www.youtube.com/embed/JyLFxCBYekE?autoplay=1';
    ...
    youTubeDiv.innerHTML = youtubePlayer;
    var loop = document.querySelector('.video-player');
    playbutton.addEventListener('click', function(){
        console.log('click');
        replace.classList.add('hide');
        if(loop) {
            loop.classList.add('hide');
        }
        videoWrapper.insertBefore(youTubeDiv, replace);
